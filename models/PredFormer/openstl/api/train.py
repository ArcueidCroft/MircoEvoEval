...
class BaseExperiment(object):
    ...
    def display_method_info(self):
        ...
        if self.args.method in ['simvp', 'tau','predformer']:
        ...
        elif self.args.method in ['swinlstm_d', 'swinlstm_b','vmrnn_d','vmrnn_b']:
            input_dummy = torch.ones(1, self.args.total_length, H, W, C).to(self.device)
    ...
    def test(self):
        ...
        eval_res, eval_log = metric(results['preds'], results['trues'],
                                    ...
                                    metrics=['mae','mse','rmse','ssim'], channel_names=channel_names, spatial_norm=spatial_norm)
        results['metrics'] = np.array([eval_res['mae'], eval_res['mse'],eval_res['rmse'],eval_res['ssim']])

        if self._rank == 0:
            print_log(eval_log)
            folder_path = osp.join(self.path, 'saved100_new')
