...
def load_data(dataname, batch_size, val_batch_size, num_workers, data_root, dist=False, **kwargs):
    cfg_dataloader = dict(
        ...
        aft_seq_length=kwargs.get('aft_seq_length', 90),
    ...
    if dataname == 'bair':
        ...
        return load_data(batch_size, val_batch_size, data_root, num_workers, **cfg_dataloader)
    elif dataname == 'Dendrite_growth':
        from .dataloader_Dendrite_growth import load_data
        return load_data(batch_size, val_batch_size, data_root, num_workers, **cfg_dataloader)
    elif dataname == 'Grain_growth':
        from .dataloader_Grain_Growth import load_data
        return load_data(batch_size, val_batch_size, data_root, num_workers, **cfg_dataloader)
    elif dataname == 'Plane_wave_propagation':
        from .dataloader_Plane_Wave  import load_data
        return load_data(batch_size, val_batch_size, data_root, num_workers, **cfg_dataloader)
    elif dataname == 'Spinodal_decomposition':
        from .dataloader_Spinodal_decomposition import load_data
        return load_data(batch_size, val_batch_size, data_root, num_workers, **cfg_dataloader)
