...
class BaseExperiment(object):
    ...
    def test(self):
        if self.args.test == False:
            ...
            ckpt = torch.load(osp.join(self.save_dir, 'checkpoints', 'best.ckpt'))  ##    self.save_dir
            # ckpt = torch.load(osp.join('work_dirs/Grain_0607_SimVP_10_50', 'checkpoints', 'best.ckpt'))
            print('********************** checkpoint loaded  ********************************')
