...
class OriginalDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        clip = self.data[sample_idx, :, :, :]  # (T, H, W)
        input_seq = clip[:self.pre_seq_len]
        output_seq = clip[self.pre_seq_len:]
        # print(torch.tensor(input_seq).unsqueeze(1).shape)
        # 
...
class LongSequenceDataset(Dataset):
    def __init__(self, data, pre_seq_len=10, aft_seq_len=10, stride=20, data_name='Dendrite_growth'):
        super().__init__()
        self.data = torch.from_numpy(data) if isinstance(data, np.ndarray) else data  # ensure torch.Tensor
        self.pre_seq_len = pre_seq_len
        self.aft_seq_len = aft_seq_len
        self.stride = stride
        self.window_size = pre_seq_len + aft_seq_len
    ...
    def __getitem__(self, index):
        ...
        input_seq = clip[:self.pre_seq_len]                       # [T, 1, H, W]
        output_seq = clip[self.pre_seq_len:]                      # [T, 1, H, W]
        return input_seq.float(), output_seq.float()
...
def load_data(batch_size, val_batch_size, data_root, 
    ...
    # test = np.load(os.path.join(data_root, 'Grain_growth/test.npz'))['data']
    # train_set = LongSequenceDataset(train, pre_seq_length, aft_seq_length      )       ## ʹ��Grain���ݼ�
    ...
    # # ! Plane 
    # print('plane')
    # train = np.load(os.path.join(data_root, 'Plane_wave_propagation/train.npz'))['data']  # [N, T, H, W]
    # val = np.load(os.path.join(data_root, 'Plane_wave_propagation/valid.npz'))['data'] 
    # test = np.load(os.path.join(data_root, 'Plane_wave_propagation/test.npz'))['data']  
    # train = train[:,:,:,:,0]
    # val = val[:,:,:,:,0]
    # train_set = SlidingWindowDataset(train, pre_seq_length, aft_seq_length , stride     )       ## ʹ��Grain���ݼ�
    # val_set = SlidingWindowDataset(val, pre_seq_length, aft_seq_length, stride         )
    # test_set = SlidingWindowDataset(test, pre_seq_length, aft_seq_length, stride)
    
    # ! Dendrite
    print('den')
    path = os.path.join(data_root, 'Dendrite_growth/train.npy')
    print(path)
    train = np.load(os.path.join(data_root, 'Dendrite_growth/train.npy'))  # [N, T, H, W]
    val = np.load(os.path.join(data_root, 'Dendrite_growth/valid.npy')) 
    test = np.load(os.path.join(data_root, 'Dendrite_growth/test.npy'))
    # test = np.load(os.path.join(data_root, 'Dendrite_growth/test_new.npy'))

    train_set = OriginalDataset(train, pre_seq_length, aft_seq_length , stride)       ## ʹ��Grain���ݼ�
    val_set = OriginalDataset(val, pre_seq_length, aft_seq_length, stride)
    test_set = OriginalDataset(test, pre_seq_length, aft_seq_length, stride)
    # pdb.set_trace()
    # print(train_set.__getitem__(10))
    # ! Spindol
    # print('spin')
    # train = np.load(os.path.join(data_root, 'Spinodal_decomposition/train.npz'))['data']  # [N, T, H, W]
    # val = np.load(os.path.join(data_root, 'Spinodal_decomposition/valid.npz'))['data'] 
    # test = np.load(os.path.join(data_root, 'Spinodal_decomposition/test.npz'))['data']
    # train_set = SlidingWindowDataset(train, pre_seq_length, aft_seq_length , stride     )       ## ʹ��Grain���ݼ�
    # val_set = SlidingWindowDataset(val, pre_seq_length, aft_seq_length, stride)
    # test_set = SlidingWindowDataset(test, pre_seq_length, aft_seq_length, stride)
    
    print("successfully loaded data")    
    #train_set = SlidingWindowDataset(train, pre_seq_length, aft_seq_length, stride)   ## ���ó�Grain��������������ݼ�
    #val_set = SlidingWindowDataset(val, pre_seq_length, aft_seq_length, stride) 
